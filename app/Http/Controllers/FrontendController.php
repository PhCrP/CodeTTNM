...
class FrontendController extends Controller
{
    ...
    public function productGrids()
    {
        ...
        if (!empty($_GET['sortBy'])) {
            $sortBy = $_GET['sortBy'];
            $sortBySecondary = !empty($_GET['sortBySecondary']) ? $_GET['sortBySecondary'] : null;

            if ($sortBy == 'title') {
                $products->where('status', 'active')->orderBy('title', 'ASC');
            } elseif ($sortBy == 'price_asc') {
                if ($sortBySecondary == 'title') {
                    $products->where('status', 'active')->orderBy('title', 'ASC')->orderBy('price', 'ASC');
                } else {
                    $products->where('status', 'active')->orderBy('price', 'ASC');
                }
            } elseif ($sortBy == 'price_desc') {
                if ($sortBySecondary == 'title') {
                    $products->where('status', 'active')->orderBy('title', 'ASC')->orderBy('price', 'DESC');
                } else {
                    $products->where('status', 'active')->orderBy('price', 'DESC');
                }
            }
        }
    ...
    public function loginSubmit(Request $request)
    {
        ...
        if (Auth::attempt(['email' => $data['email'], 'password' => $data['password'], 'status' => 'active'])) {
            ...
            // Kiểm tra tham số redirect
            if ($request->has('redirect') && filter_var($request->input('redirect'), FILTER_VALIDATE_URL) && Str::startsWith($request->input('redirect'), url('/'))) {
    ...
    public function subscribe(Request $request)
    {
        if (! Newsletter::isSubscribed($request->email)) {
            ...
            if (Newsletter::lastActionSucceeded()) {
                ...
                return back()->with('error', 'Có gì đó không ổn! Vui lòng thử lại sau!');
            }
        } else {
